...
class MenuRoleCon(Base):
    # 表名
    __tablename__ = "menu_role_con"
    __table_args__ = {
        'mysql_charset': 'utf8'
    }
    # 表结构
    menu_id = Column(String(64), ForeignKey("menu.id"))    # 菜单id
    role_id = Column(String(64), ForeignKey("role.id"))    # 用户id
    ...
    menu = relationship("Menu", backref="menu_role_con_list")
    role = relationship("Role", backref="menu_role_con_list")
